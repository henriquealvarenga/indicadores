...
# Fonte: IBGE — SIDRA 7358 — Projeções por grupo de idade e sexo
# Ordem etária canônica (grupos quinquenais + grupo terminal aberto), garante
# o empilhamento correto da pirâmide independentemente da forma do dado de origem.
faixas_ordem <- c(
...
piramide_raw <- carregar_dados("piramide_brasil") |>
  mutate(faixa = factor(as.character(faixa), levels = faixas_ordem)) |>
  filter(ano %in% c(2000, 2030), !is.na(faixa))
