...
::: {.formula-box}
**TFT** = Σ (Taxa de fecundidade específica por idade) × amplitude do grupo etário

*O fator é a largura do intervalo etário: **× 5** quando se usam grupos quinquenais (15–19, 20–24, …); **× 1** para taxas por idade simples.*
:::
...
#| fig-cap: "Evolução da Taxa de Fecundidade Total no Brasil — projeções IBGE"
...
::: {.callout-note}
## Projeções, não dados observados

As séries de fecundidade, natalidade, mortalidade e estrutura etária do IBGE usadas neste livro (capítulos 2, 3 e 7) vêm das **Projeções da População — Revisão 2018**: estimativas demográficas **modeladas**, e não medições diretas dos registros de nascimentos e óbitos. Elas captam bem a **tendência** de longo prazo, mas não refletem choques conjunturais (como a COVID-19). Para os valores **observados** ano a ano, consulte o SINASC e o SIM via [TabNet/DATASUS](http://tabnet.datasus.gov.br/).
:::
...
#| fig-cap: "TFT por Grande Região — projeções IBGE"
...
#| fig-cap: "Taxa Bruta de Natalidade por região do Brasil — projeções IBGE"
